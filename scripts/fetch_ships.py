...
# Helper to get Country from MMSI (First 3 digits = MID code)
def get_flag(mmsi):
    try:
        mid = int(str(mmsi)[:3])
        # A simplified list of common MIDs
        if 201 <= mid <= 279: return "Europe"
        if 303 <= mid <= 374: return "North/Central America"
        if 401 <= mid <= 478: return "Asia"
        if 501 <= mid <= 578: return "Oceania"
        if 601 <= mid <= 679: return "Africa"
        if 701 <= mid <= 775: return "South America"
        ...
        while datetime.now().timestamp() < end_time:
            try:
                ...
                if data['MessageType'] == 'PositionReport':
                    ...
                    ships[mmsi]['heading'] = msg.get('TrueHeading', 511)
